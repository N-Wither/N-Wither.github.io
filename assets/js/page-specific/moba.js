...
let themeSelector = document.getElementById('scoreboard-theme')
...
function setScore(num, team) {
    ...
    let scoreHTML = 
    stringMultiply(`<div class="score-${team}" style="background:${scoreboardThemes[currentTheme][`${team}Color`]}"></div>`, score) + 
    stringMultiply(`<div class="score-blank"></div>`, scoreToWin - score)
...
function refreshBoard() {
    ...
    themeSelector.addEventListener(('change', 'input'), () => {
        ...
        blues.forEach(ele => {
            ele.style.color = scoreboardThemes[theme].blueColor
        })
        reds.forEach(ele => {
            ele.style.color = scoreboardThemes[theme].redColor
        })
        bg.forEach(ele => {
            ele.style['background-color'] = scoreboardThemes[theme].backgroundColor
            ele.style.color = scoreboardThemes[theme].textColor
        })
        blueLogoBg.style.background = scoreboardThemes[theme].blueLogo
        redLogoBg.style.background = scoreboardThemes[theme].redLogo
        blueScores.forEach(ele => {
            ele.style['background-color'] = scoreboardThemes[theme].blueColor
        })
        redScores.forEach(ele => {
            ele.style['background-color'] = scoreboardThemes[theme].redColor
        })
    })
